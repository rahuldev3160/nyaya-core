...
- [RISK-09](risks.md#risk-09) — EPFO's 3-bucket vs. PFRDA's 7-bucket real English taxonomies can't be merged either direction without inventing unevidenced categories or discarding real signal — RESOLVED same session, Rahul chose to leave them separate
- [DECIDE-38](decisions.md#decide-38) — Cloud Run hosting scaffold built for nyaya-core (bind change, Dockerfile, DEPLOY.md) on an unmerged `infra/cloud-run-scaffold` branch; real auth gap found (Devthorium's client sends no identity token, will 403 once IAM-restricted) not yet fixed; FOUNDATION.md's public-reachability approval gate not yet cleared

---

**Next available IDs:** DECIDE-39 · BUG-16 · RESEARCH-13 · RISK-10 · ASSUME-02
