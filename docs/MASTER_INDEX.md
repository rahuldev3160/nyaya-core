...
- [DECIDE-37](decisions.md#decide-37) — Built Phase 2 (hybrid retrieval + FastAPI: `/exams /papers /topics /search /pyq /topic/{id}/brief /verify_citation /ingest` + new `/attempt`); `scripts/inventory.py` immediately found PFRDA has zero indexed chunks (EPFO only 125 thin) — `/pyq` is the real foundation for both, not `/search`. PFRDA/EPFO practice in Devthorium sequenced as a new additive path, not a full Recall cutover (Devthorium's hardcoding spans 30+ files; nothing to migrate from for these two exams)
- [BUG-15](bugs.md#bug-15) — `pyq_bank.options` stored as a list for rbi_depr/EPFO vs. a dict for pfrda_gradea — `scripts/quiz.py`'s `print_question()` would have crashed on every rbi_depr/EPFO row; fixed via shared `normalize_options()`
- [RESEARCH-12](research.md#research-12) — Remaining EPFO/RBI-DSIM/PFRDA cross-exam topic overlap audit: one real substantial candidate found (objective English-test content, 132 real PYQs across both exams) blocked on a genuine granularity mismatch (RISK-09); commerce/accountancy and current-affairs candidates checked and explicitly rejected (real depth/flavor mismatches); IES found to carry 2 likely-duplicate zero-content topic_ids alongside their already-linked canonical equivalents, flagged not merged
- [RISK-09](risks.md#risk-09) — EPFO's 3-bucket vs. PFRDA's 7-bucket real English taxonomies can't be merged either direction without inventing unevidenced categories or discarding real signal — RESOLVED same session, Rahul chose to leave them separate, no merge
- [DECIDE-38](decisions.md#decide-38) — Persisted, indexed AI-generated question bank stored in `pyq_bank` (`source_type='ai_generated'`), generation pipeline lives in nyaya-core (not Devthorium, deviating from Devthorium's unmerged ephemeral PR #58); serves immediately without a human-review gate, badge is the honesty mechanism; no external sourcing
- [BUG-16](bugs.md#bug-16) — `max_tokens=4096` too tight once claude-sonnet-5's `ThinkingBlock` competes for the same budget, truncated JSON mid-generation (BUG-14's exact failure class) — fixed, raised to 8192
- [BUG-17](bugs.md#bug-17) — Retry logic treated a non-retryable Anthropic billing error (`credit balance too low`) the same as a transient failure, burning ~300 slots' worth of guaranteed-to-fail retries before the run was noticed and killed manually — fixed, now fails the whole run fast on this specific error
- [BUG-18](bugs.md#bug-18) — ~35% (158/454) of PFRDA's real MCQ rows have broken/incomplete `options` (mostly the 2025 paper's reasoning/puzzle section) — found live via Rahul's first real drill session; worked around at Devthorium's serving layer (`_is_answerable_mcq` filter), not fixed at the source; re-extraction from source PDF is real, scoped, unstarted work
- [RISK-10](risks.md#risk-10) — Full-scale AI generation run hit the Anthropic account's credit balance (271 PFRDA questions written for $5.55 before it ran dry, 0 EPFO) — idempotent to resume, but Rahul explicitly declined to add credits for now; no further action pending

---

**Next available IDs:** DECIDE-39 · BUG-19 · RESEARCH-13 · RISK-11 · ASSUME-02
