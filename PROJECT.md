...
LanceDB) built, tested, and validated against real content. 9 exams registered
(`upsc_cse`, `upsc_ies`, `rbi_gradeb`, `rbi_depr`, `uppcs`, `hpas`, `ukpsc`, `mppsc`,
`upsc_epfo_apfc_eo_ao`) — naming finalized as `{institution}_{exam}`, papers scoped by a
composite key (DECIDE-21). `upsc_cse`'s Optional papers are split into Paper I/II
(`eco_optional_1/2`, `law_optional_1/2` — DECIDE-25). 8 exams have real seeded topic
taxonomies (`upsc_cse`'s non-prelims/non-optional papers and `rbi_depr` don't yet). First
real ingestion run (`upsc_epfo_apfc_eo_ao`'s 2025 paper) yielded 36 verified questions
across 14 topics — see `HANDOFF.md` for what's next.
